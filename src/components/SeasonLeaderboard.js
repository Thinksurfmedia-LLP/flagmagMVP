...
// seasonsParam: comma-separated season IDs
export default function SeasonLeaderboard({ orgSlug, seasonsParam }) {
    ...
    useEffect(() => {
        ...
        Promise.all(
            ALL_TYPES.map((type) =>
                fetch(
                    `/api/organizations/${orgSlug}/seasons/leaderboard?seasons=${encodeURIComponent(seasonsParam)}&statType=${type}`
                )
                    .then((r) => r.json())
